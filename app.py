    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": "Please create a comprehensive class schedule for the upcoming semester that includes the following classes and their respective times. Ensure there are no overlapping classes, and every selected class is consistent in timing throughout the semester. If there are any overlaps, please select alternative options or omit one of the conflicting classes. Each class must only occur on the specified days. The classes are in the Format 'Class, Days(MoTuWeThFr),Startime, endtime, recitation day, recitation start time, recitation end time " + class_gpt + ". Steps to follow exactly. do not change the steps. 1. Chose class 2. See dates and times and chose a time option of times and fill out the schedule for every date assigned for that class. 3. go to next class 4. Select that classes date and time 5. if that class would overlap with a class currently on the schedule then chose another time given. If that time also overlaps, then go back to the other class and remove it from the schedule and chose another time given and fill the schedule back out for all times day days for that class. If then the chosen class has no overlaps then add it to the schedule and move to step 3. (ex a class from 10-11 overlaps with a class from 10:30-11:30) (ex a class from 10-11 also overlaps with a class from 11-12) please give me the schedule in the format 'Mon Class(Times)!Class(Times)!,Tues Class(Times)!Class(Times)!,Wed Class(Times)!Class(Times)!,Thur Class(Times)!Class(Times)!,Fri Class(Times)!Class(Times)!' There should be no spaces in this output, except for a single space between the day of the week and the start of the class list. This output should be one line and the only thing ever returned. if a day has no classes just say the name of the day with no classes afterwards for every day of the week. If a class does not fit in schedule after running algorithm just leave it out."}
            ]
        )

        output = response.choices[0].message.content
        days = output.split(',')
        monday_classes = days[0].split(' ')[1].split('!')
        print(monday_classes)
        tuesday_classes = days[1].split(' ')[1].split('!')
        print(tuesday_classes)
        wednesday_classes = days[2].split(' ')[1].split('!')
        print(wednesday_classes)
        thursday_classes = days[3].split(' ')[1].split('!')
        print(thursday_classes)
        friday_classes = days[4].split(' ')[1].split('!')
        print(friday_classes)

        return render_template('ai_scheduler.html', mo=monday_classes, tu=tuesday_classes, we=wednesday_classes, th=thursday_classes, fr=friday_classes)
    except:
        return "There was an issue"
